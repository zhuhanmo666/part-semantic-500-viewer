...
function isManual(source) { return source === 'vlm_plus_manual_review' || source === 'manual'; }
function sourceLabel(source) { return isManual(source) ? 'VLM + 人工审核' : 'VLM 自动通过'; }
...
function renderModels() {
  ...
  ui.modelList.replaceChildren(...state.filtered.map((row) => {
    ...
    button.innerHTML = `<strong>${row.asset_id}</strong><small><span>${row.part_count} parts</span><span class="${isManual(row.annotation_source) ? 'manual-badge' : ''}">${isManual(row.annotation_source) ? '人工审核' : 'VLM'}</span></small>`;
...
function renderPartDetail() {
  ...
  ui.semanticSource.textContent = part ? (part.manual_annotation || part.semantic_source === 'manual' ? '人工审核' : 'VLM') : '—';
}
...
async function selectModel(row) {
  ...
  try {
    ...
    ui.annotationSource.textContent = sourceLabel(annotation.annotation_source);
    ui.partCount.textContent = annotation.part_count;
    const mergeStatus = annotation.semantic_merge_status || annotation.semantic_merge_result;
    ui.mergeStatus.textContent = mergeStatus === 'merged_verified' ? '已合并并验证' : (mergeStatus === 'no_merge' ? '无需合并' : (mergeStatus || '未合并'));
    ui.partSearch.value = '';
    renderParts();
    renderPartDetail();
    updateUrl();
...
async function init() {
  try {
    ...
    ui.statManual.textContent = data.summary.manually_reviewed_asset_count.toLocaleString();
    renderModels();
